...
export function AllMarketsSheet({ isOpen, onClose, markets, mids, marketStats, onSelect }: AllMarketsSheetProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return markets.filter(({ market }) =>
      getMarketSearchTerms(market).some((term) => term.toLowerCase().includes(q))
    );
  }, [markets, query]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col">
      ...
      <div className="relative mt-auto max-h-[92vh] flex flex-col rounded-t-2xl bg-white animate-slide-up">
